...
- **The stepper's number is a real input now, not a span — tap it to type a value.** `bump(dir)`
  tracks one boolean, `pressed`: the first press since landing on a value (typed, or a fresh
  stepper) is a whole-number `step` (default 1); every press after that is a quarter-step
  `fineStep` (default 0.25) — in *either* direction. An earlier version made the fine step
  conditional on reversing, which sounds reasonable but is not what it feels like to use: press
  the same button twice and the second press silently jumps by a whole number again, which
  reads as broken once you notice it. One rule, no direction-dependent special case, is the
  right design here — don't reintroduce the reversal-only version. Typing a value directly, or
  even clearing the box to empty (still routes through `change`), resets `pressed` to false, so
  the next button press starts fresh at a whole step. `focus` selects all the text so typing
  overwrites rather than appends; commit is on `change` (fires on blur), matching `kcalIn` in
  the targets sheet, not `input`+`blur` like the grams fields — there is nothing here that needs
  to update live as you type. The global `input,select,textarea` rule (padding,
  `min-height:44px`, `width:100%`, a border) is reset back down for `.stepper .sv` specifically,
  the same trap `.checks input[type=checkbox]` hit earlier. Every `stepper()` call in the app is
  a servings/portion field, so this is one shared component, not a per-screen choice — do not
  give one call site a different `step` without a reason; the four that existed before this were
  an accidental inconsistency, not a design.
...
- **Energy out is blue (`--move`), energy in is brass, and nothing nets.** `ringSVG(pct, outPct)`
  draws two rings on one scale (the calorie target): intake clockwise from the top, burn
